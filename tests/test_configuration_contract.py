    ...
    def test_every_pipeline_env_is_exposed_to_build_and_report(self) -> None:
        ...
        for name, default in PIPELINE_ENV_DEFAULTS.items():
            if name == "MAX_MEDIA_DOWNLOADS":
                ...
                self.assertIn(
                    f"MAX_MEDIA: ${{{{ vars.MAX_MEDIA_DOWNLOADS || '{default}' }}}}",
                    workflow,
                )
                self.assertIn(
                    f"MAX_MEDIA_DOWNLOADS: ${{{{ vars.MAX_MEDIA_DOWNLOADS || '{default}' }}}}",
                    workflow,
                )
                continue
            self.assertGreaterEqual(workflow.count(f"{name}:"), 2, name)

    def test_media_policy_defaults_match_canonical_python_defaults(self) -> None:
        workflow = Path(".github/workflows/build-video-kit.yml").read_text(encoding="utf-8")
        relevance = PIPELINE_ENV_DEFAULTS["MEDIA_MIN_RELEVANCE_SCORE"]
        self.assertIn(
            f"MEDIA_MIN_RELEVANCE_SCORE: ${{{{ vars.MEDIA_MIN_RELEVANCE_SCORE || '{relevance}' }}}}",
            workflow,
        )
