...
class ReviewHubCatalogTests(unittest.TestCase):
    ...
    def test_build_catalog_adds_left_sidebar_and_query_driven_episode_switching(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('id="repoHealthLink"', document)
            self.assertIn('href="?view=health"', document)
            self.assertIn('value="__health__"', document)
            self.assertIn('id="memoryLink"', document)
            self.assertIn('href="?view=memory"', document)
            self.assertIn('value="__memory__"', document)
            self.assertIn('id="metricsLink"', document)
            self.assertIn('href="?view=metrics"', document)
            self.assertIn('value="__metrics__"', document)
            self.assertIn("params.get('episode')", document)
            self.assertIn("params.get('view')==='health'", document)
            self.assertIn("params.get('view')==='memory'", document)
            self.assertIn("params.get('view')==='metrics'", document)
