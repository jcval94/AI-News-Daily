...
STAGES: list[dict[str, Any]] = [
    ...
    {"id": "resolve_bridge", "kind": "deterministic", "title": "DaVinci Resolve Bridge v0", "summary": "Construye un plan CI-testable con bins, tracks, imports, placements por frame y markers. La ejecución local usa la API de Resolve y rehúsa sobrescribir timelines existentes.", "inputs": "virtual_timeline.json + placeholder_manifest.json + multimedia resuelta", "outputs": "resolve_bridge_plan.json; execution JSON solo local con --execute", "authority": "Plan: Python determinista; ejecución: DaVinciResolveScript local", "code": "pipeline/resolve_bridge.py · config/resolve_bridge_plan.schema.json · docs/resolve_bridge.md", "trace_steps": []},
    ...
    {"id": "report_promote", "kind": "deterministic", "title": "Estado, trazas, reporte y promoción", "summary": "Persiste evidencia del run y solo promueve si script, Production Script, Recording Pack, Virtual Timeline, Placeholder Media, Resolve Bridge plan, OTIO round-trip, reporte y multimedia densa terminaron correctamente.", "inputs": "artefactos + gate final + resultado multimedia + recording/edit handoff", "outputs": "run_state + execution_trace + run_report + canon opcional", "authority": "Python/GitHub Actions", "code": "pipeline/report.py · build-video-kit.yml", "trace_steps": []},
...
DECISION_FLOW = [
    ...
    "Virtual Timeline → placeholder media físico → Resolve Bridge v0 plan",
