...
STAGES: list[dict[str, Any]] = [
    ...
    {"id": "media_materialize", "kind": "service", "title": "Materialización y gate multimedia", "summary": "Descarga proveedores con retries acotados y exige >=45 assets y >=5 en los primeros 20 s. El pool documental optativo agrupa necesidades y valida identidad, derechos y archivos antes de asignar; conserva faltantes críticos y separa alternativas del montaje.", "inputs": "plan denso + queries", "outputs": "assets + manifest + credits + zip", "authority": "Python/GitHub Actions", "code": "pipeline/media.py · pipeline/media_pool.py · pipeline/media_acquisition.py · pipeline/review_media_offline_dense.py · build-video-kit.yml", "trace_steps": []},
    ...
    {"id": "local_harness", "kind": "local_control", "title": "Windows Local Editing Harness", "summary": "Frontera segura entre contratos canónicos y la workstation Windows 11: doctor/capabilities, toolchain snapshot, roots lógicos, requests versionados que deben pasar a staging privado con SHA-256, jobs allowlisted, receipts idempotentes, Resolve discovery/OTIO smoke y ejecución sin shell arbitrario.", "inputs": "config/local + .local/local_config.json + jobs declarativos", "outputs": "local_environment + local_capabilities + local_job/local_receipt + .local/preflight.latest.json + efectos verificados en Resolve", "authority": "Python/PowerShell deterministas; nunca expone shell arbitrario y nunca sube raw media", "code": "pipeline/local/* · config/local/* · scripts/local/* · docs/local/README.md", "trace_steps": []},
...
DESIGN_DECISIONS = [
    ...
    ("Git propone; Windows acepta", "local_handoff/requests nunca se autoejecuta. El usuario debe stagear el request a .local/jobs/staged, se fija SHA-256 y la ejecución conserva doble consentimiento local."),
