...
from experiments.video_search.providers import safe_error
...
def plot_replay(data, path):
    ...
    bins = data["bins"]
    times = [(b["start_time"] + b["end_time"]) / 120 for b in bins]
    values = [b["value"] for b in bins]
    fig, ax = plt.subplots(figsize=(10, 2.6), layout="constrained")
    ax.fill_between(times, values, color="#7164d8", alpha=.45)
    ax.plot(times, values, color="#5144ad", linewidth=1.5)
    ax.set(xlabel="Minutes in original video", ylabel="Relative replay intensity", ylim=(0, 1.05),
           title="YouTube · Most Replayed (published data)")
